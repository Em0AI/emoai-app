...
import utils
from core import emotion_module
from core import rag_module
from core import agent_module
from core import meta_module
import datetime
...
def chat_fn(user_input, chat_history, tone=0.6, agent_override=None,emotion_history=None):
    ...
    if meta_key:
        ...
        print(f"🧭 find Meta feedback：{meta_meaning}，update → {user_prefs}")

    # --- 情绪识别 (调用 emotion_module) ---
    prev_val = emotion_history[-1][2] if emotion_history else None
    
    emo, score, adj_val_raw = emotion_module.detect_emotion(user_input)
    ...
    # (删除原版 emoGen3.py 中 chat_fn 末尾的多余 return 和 chat_history.append)
